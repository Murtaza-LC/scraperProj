// netlify/functions/scrape.js  (CommonJS for Netlify Functions)
// Requires: Playwright installed and Chromium vendored via netlify.toml build command.
...
const pctOff = (mrp, price) => (mrp && price && mrp > 0 && price <= mrp)
  ? Math.round((100 * (mrp - price) / mrp) * 10) / 10
  : null;
...
module.exports.handler = async function (event) {
  try {
    ...
    // PDP enrichment is OFF by default to avoid timeouts:
    // set pdpPrices=true to enable (and then we’ll add enrichment in a later iteration)
    ...
    // (Optional) PDP enrichment (disabled by default here to reduce runtime)
    // if (pdpPrices && out.length) { /* add PDP enrichment here in a later pass */ }
